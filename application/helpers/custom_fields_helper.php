...
function render_custom_fields($belongs_to, $rel_id = false, $where = [], $items_cf_params = [])
{
    ...
    if (count($fields)) {
        ...
        foreach ($fields as $field) {
            ...
            }elseif ($field['type'] == 'contact') {
                $t = $field['type'] == 'text';
                $valueName= "";
                if($value){
                    $CI->db->where('id', $value);
                    $contact_related_to = $CI->db->get('tblcontacts')->row();
                    $valueName = $contact_related_to->firstname . ' ' . $contact_related_to->lastname;
                }
                $fields_html .= render_related_to_contact('custom_fields[' . $field['fieldto'] . '][' . $field['id'] . ']', $field_name, $value,$valueName, $t, $_input_attrs);
            }elseif ($field['type'] == 'company') {
                $t = $field['type'] == 'text';
                $valueName= "";
                if($value){
                    $CI->db->where('userid', $value);
                    $company_related_to = $CI->db->get('tblclients')->row();
                    $valueName = $company_related_to->company;
                }
                $fields_html .= render_related_to_company('custom_fields[' . $field['fieldto'] . '][' . $field['id'] . ']', $field_name, $value,$valueName, $t, $_input_attrs);
...
function is_related_to_contact($field){
...
function is_related_to_company($field){
