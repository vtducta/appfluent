...
function policy_profile_image_url($policy_id, $type = 'small')
{
    $url  = base_url('assets/images/user-placeholder.jpg');
    $CI   = & get_instance();
    $path = $CI->object_cache->get('policy-profile-image-path-' . $policy_id);

    if (!$path) {
        $CI->object_cache->add('policy-profile-image-path-' . $policy_id, $url);

        $CI->db->select('profile_image');
        $CI->db->from('tblpolicies');
        $CI->db->where('id', $policy_id);
        $policy = $CI->db->get()->row();

        if ($policy && !empty($policy->profile_image)) {
            $path = 'uploads/policy_profile_images/' . $policy_id . '/' . $type . '_' . $policy->profile_image;
            $CI->object_cache->set('policy-profile-image-path-' . $policy_id, $path);
        }
    }
