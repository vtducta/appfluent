...
function handle_contact_attachments_upload($id, $customer_upload = false)
{
    $path = get_upload_path_by_type('contact') . $id . '/';
    $CI =& get_instance();
    if (isset($_FILES['file']['name'])) {
        do_action('before_upload_client_attachment', $id);
        // Get the temp file path
        ...
        if (!empty($tmpFilePath) && $tmpFilePath != '') {
            ...
            if (move_uploaded_file($tmpFilePath, $newFilePath)) {
                ...
                $CI->misc_model->add_attachment_to_database($id, 'contacts', $attachment);
            }
        }
    }
}
...
function handle_contact_profile_image_upload2($contact_id = '')
{
    if (isset($_FILES['contact']['name']['profile_image']) && $_FILES['contact']['name']['profile_image'] != '') {
        ...
        // Get the temp file path
        $tmpFilePath = $_FILES['contact']['tmp_name']['profile_image'];


        // Make sure we have a filepath
        if (!empty($tmpFilePath) && $tmpFilePath != '') {
            // Getting file extension
            $path_parts         = pathinfo($_FILES['contact']["name"]['profile_image']);
            $extension          = $path_parts['extension'];
            $extension = strtolower($extension);
            ...
            _maybe_create_upload_path($path);
            $filename    = unique_filename($path, $_FILES['contact']["name"]['profile_image']);
            $newFilePath = $path . $filename;
            // Upload the file into the company uploads dir
            if (move_uploaded_file($tmpFilePath, $newFilePath)) {
                $CI =& get_instance();
...
function get_upload_path_by_type($type)
{
    switch ($type) {
        ...
        break;
        case 'policy_profile_images':
            return POLICY_PROFILE_IMAGES_FOLDER;
        break;
        case 'newsfeed':
...
function handle_policy_profile_image_upload($policy_id = '')
{

    if (isset($_FILES['policy']['name']['profile_image']) && $_FILES['policy']['name']['profile_image'] != '') {

        do_action('before_upload_policy_profile_image');

        $path        = get_upload_path_by_type('policy_profile_images') . $policy_id . '/';
        // Get the temp file path
        $tmpFilePath = $_FILES['policy']['tmp_name']['profile_image'];


        // Make sure we have a filepath
        if (!empty($tmpFilePath) && $tmpFilePath != '') {
            // Getting file extension
            $path_parts         = pathinfo($_FILES['policy']["name"]['profile_image']);
            $extension          = $path_parts['extension'];
            $extension = strtolower($extension);
            $allowed_extensions = array(
                'jpg',
            ...
            _maybe_create_upload_path($path);
            $filename    = unique_filename($path, $_FILES['policy']["name"]['profile_image']);
            $newFilePath = $path . $filename;
            // Upload the file into the company uploads dir
            if (move_uploaded_file($tmpFilePath, $newFilePath)) {
                $CI =& get_instance();
                ...
                $CI->db->where('id', $policy_id);
                $CI->db->update('tblpolicies', array(
                    'profile_image' => $filename
                ));
                // Remove original image
                unlink($newFilePath);
