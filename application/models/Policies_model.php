...
class Policies_model extends CRM_Model
{
    ...
    public function get_structure_policy(){
        return $this->db->field_data('tblpolicies');
    }
    /**
    ...
    public function get_policy($id)
    {
        ...
        return $this->db->get('tblpolicies')->row();
    }
    ...
    public function update_move_policy($policy_id,$client_id){
        $data = array();
...
//        }
    ...
    */
    ...
    public function get_policy_activity_log($id)
    {
        $sorting = do_action('lead_policy_log_default_sort','DESC');

        $this->db->where('policy_id', $id);
        $this->db->order_by('date', $sorting);

        return $this->db->get('tblpolicy_activity_log')->result_array();
    }

    public function log_policy_activity($policyid, $description, $integration = false, $additional_data = '', $custom_activity =0)
    {
        $log = array(
        ...
        $this->db->insert('tblpolicy_activity_log', $log);
    ...
    public function update_policy($data, $id, $client_request = false)
    {
        if (isset($data['fakeusernameremembered'])) {
            unset($data['fakeusernameremembered']);
        }
        ...
        $hook_data = do_action('before_update_policy', ['data' => $data, 'id' => $id]);
        $data      = $hook_data['data'];
        ...
        if ($affectedRows > 0) {
            logActivity('Policy Updated [ID: ' . $id . ']');
    ...
    public function add_policy($data,  $client_request = false)
    {
        if (isset($data['fakeusernameremembered'])) {
            unset($data['fakeusernameremembered']);
        }
        ...
        $hook_data['data'] = $data;
        $hook_data         = do_action('before_add_policy', $hook_data);
        $data              = $hook_data['data'];
        ...
        $hook_data = do_action('after_add_policy', ['data' => $data, 'id' => $id]);
        $data      = $hook_data['data'];

        if ($id > 0) {
            logActivity('Policy Insert [ID: ' . $id . ']');
