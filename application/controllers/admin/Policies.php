...
class Policies extends Admin_controller
{
    ...
    public function index()
    {
        ...
        $data['groups']         = $this->policies_model->get_groups();
        ...
        $data['customer_admins'] = $this->policies_model->get_customers_admin_unique_ids();
        ...
        $data['contacts_logged_in_today'] = $this->policies_model->get_contacts('', 'last_login LIKE "'.date('Y-m-d').'%"'.$whereContactsLoggedIn);


        $arrPolicyField = $this->policies_model->get_structure_policy();


        $arrFieldStandard =  $this->unsetObject($arrPolicyField,'firstname,lastname');
        ...
        $data['policies_structure'] =  $this->unsetObject($arrPolicyField,'firstname,lastname');

        $arrPolicyCustomField = get_custom_fields('policies');
        foreach ($arrPolicyCustomField as $item){
            $dataItem = array();
            $dataItem["name"] = $item['name'];
            $dataItem["type"] = "custome_field_policies";
            $dataItem["id"] = $item['id'];
        ...
        $data['policies_custom_field_structure'] =$arrPolicyCustomField;
...
//        $arrCampaign = $this->Newsletter_model->getCampaigns();
    ...
    public function table()
    {
        ...
        $this->app->get_table_data('policies');
    }
...
//            ));
    ...
    public function policy($id = '')
    {
        ...
        if ($this->input->post() && !$this->input->is_ajax_request()) {
            if ($id == '') {
                ...
                $data_policy = $datapost['policy'];
                unset($datapost['policy']);
                $datamerge =  $datapost;

                $data = array_merge($datamerge,$data_policy);
                unset($data['policyid']);
                ...
                $id = $this->policies_model->add_policy($data);

                handle_policy_profile_image_upload($id);

                $search_client = $this->policies_model->get_by_name($client_name);

                //update company
                if($search_client){
                    $new_id = $search_client->userid;
                    $this->policies_model-> update_move_policy ($id,$new_id);
                }else{ //khong tim thay
                    if($client_name){
                        $data_client['is_client'] =0;
                        $data_client['company'] = $client_name;
                        $new_id = $this->policies_model->add($data_client);
                        $this->policies_model-> update_move_policy ($id,$new_id);
                    }
                }

                handle_tags_save($tags, $id, 'policy');

                if($id>0){
                    $this->policies_model->log_policy_activity($id, 'not_policy_activity_created', false, serialize(array(
                        get_staff_full_name(get_staff_user_id())
                    )));

                    set_alert('success', _l('added_successfully', _l('policy')));
                        redirect(admin_url('policies/policy/' . $id));
                }
            } else {
                ...
                //var_dump($data);die;
                $this->policies_model->update_policy($data, $id);
                ...
                $this->policies_model->log_policy_activity($id, 'not_policy_activity_updated', false, serialize(array(
                        get_staff_full_name(get_staff_user_id())
                )));
                redirect(admin_url('policies/policy/' . $id));
            }
        ...
        if ($id == '') {
            $title = _l('add_new', _l('policy_lowercase'));
        } else {

            $data['policy'] = $this->policies_model->get_policy($id);

            //$data['contacts']         = $this->policies_model->get_contacts($id);
            if ($data['policy']) {
                $data['policyid']=$data['policy']->id;
                $data['policy_tags'] = implode(',',get_tags_in($data['policyid'],'policy'));
            ...
            if ($group == 'profile_families') {
                ...
                $data['activity_log']  = $this->policies_model->get_policy_activity_log($data['policy']->id);
                ...
                //$contact = $this->policies_model->get_contact(get_primary_contact_user_id($id));
                ...
                $section = $this->custom_sections_model->get_by_section_slug($slug,'policies');
            ...
            $title                 = $data['policy']->firstname . ' ' .$data['policy']->lastname;
        ...
        $this->load->model('custom_sections_model');
        $arraySection = $this->custom_sections_model->get(false,'policies');
        ...
        $this->load->view('admin/policies/client_family', $data);
    }
    ...
    public function contact($customer_id, $contact_id = '')
    {
        ...
        if ($this->input->post()) {
            ...
            if ($contact_id == '') {
                ...
                $id      = $this->policies_model->add_contact($data, $customer_id);
                $message = '';
                $success = false;
                if ($id) {
                    handle_contact_profile_image_upload($id);
                    $success = true;
                    $primary_contact_id = $this->policies_model->get_contact_primary($customer_id)->id;
                    $this->policies_model->log_contact_activity($primary_contact_id , 'not_contact_activity_created_related_contact', false, serialize(array(
                        get_staff_full_name(get_staff_user_id())
                    )));
                ...
                $original_contact = $this->policies_model->get_contact($contact_id);
                $success          = $this->policies_model->update_contact($data, $contact_id);
                $message          = '';
                ...
                if ($updated == true) {
                    $contact = $this->policies_model->get_contact($contact_id);
                ...
                if($success){
                    $primary_contact_id = $this->policies_model->get_contact_primary($customer_id)->id;
                    $this->policies_model->log_contact_activity($primary_contact_id , 'not_contact_activity_updated_related_contact', false, serialize(array(
                        get_staff_full_name(get_staff_user_id())
                    )));
                }
                echo json_encode(array(
        ...
        if ($contact_id == '') {
            ...
            $data['contact'] = $this->policies_model->get_contact($contact_id);
        ...
        $data['customer_permissions'] = $this->app->get_contact_permissions();
        $data['title']                = $title;
        $this->load->view('admin/policies/modals/contact_family', $data);
    }
    ...
    public function delete_policy_profile_image($policy_id)
    {
        do_action('before_remove_policy_profile_image');
        if (file_exists(get_upload_path_by_type('policy_profile_images') . $policy_id)) {
            delete_dir(get_upload_path_by_type('policy_profile_images') . $policy_id);
        }
        $this->db->where('id', $policy_id);
        $this->db->update('tblpolicies', array(
            'profile_image' => null,
        ));
    }
    ...
    public function assign_admins($id)
    {
        ...
        $success = $this->policies_model->assign_admins($this->input->post(), $id);
        if ($success == true) {
            $primary_contact_id = $this->policies_model->get_contact_primary($id)->id;
            $this->policies_model->log_contact_activity($primary_contact_id , 'not_contact_activity_assign_admin', false, serialize(array(
                get_staff_full_name(get_staff_user_id())
            )));
    ...
    public function delete_contact($customer_id, $id)
    {
        ...
        $this->policies_model->delete_contact($id);
...
//        $this->perfex_base->get_table_data('client_families');
    ...
    {
        if (has_permission('customers', '', 'delete')) {
            $this->policies_model->delete_attachment($id);
        }
        redirect($_SERVER['HTTP_REFERER']);
    }
    ...
    public function delete($contact_id)
    {
        ...
        $response = $this->policies_model->delete_contact($contact_id);
    ...
    {
        if (is_admin()) {
            $this->policies_model->login_as_client($id);
        }
    ...
    {
        echo json_encode($this->policies_model->get_customer_billing_and_shipping_details($id));
    }
    ...
    public function change_contact_status($id, $status)
    {
        if (has_permission('customers', '', 'edit') || is_customer_admin(get_user_id_by_contact_id($id))) {
            if ($this->input->is_ajax_request()) {
                $this->policies_model->change_contact_status($id, $status);
            }
        }
    }
    ...
    {
        if ($this->input->is_ajax_request()) {
            $this->policies_model->change_client_status($id, $status);
        }
    }
    ...
    public function import()
    {
                                ...
                                if (!$this->input->post('simulate')) {
                                    ...
                                    if (isset($insert['company']) && $insert['company'] != '' && $insert['company'] != '/') {
                                        if (total_rows('tblclients', array('company'=>$insert['company'])) === 1) {
                                            ...
                                            $contactid = $this->policies_model->add_contact($tmpInsert, $existingCompany->userid, true);
                                    ...
                                    $clientid                        = $this->policies_model->add($insert, true);
                                    if ($clientid) {
                                        ...
                                        if (!has_permission('customers', '', 'view')) {
                                            ...
                                            $this->policies_model->assign_admins($assign, $clientid);
                                        }
                                    }
                                } else {
        ...
        $this->load->view('admin/policies/import', $data);
    }
    ...
    public function group()
    {
        if ($this->input->is_ajax_request()) {
            $data = $this->input->post();
            if ($data['id'] == '') {
                $success = $this->policies_model->add_group($data);
                $message = '';
                if ($success == true) {
                ...
                $success = $this->policies_model->edit_group($data);
                $message = '';
                if ($success == true) {
    ...
    public function delete_group($id)
    {
        ...
        $response = $this->policies_model->delete_group($id);
        if ($response == true) {
            ...
            if (is_array($ids)) {
                foreach ($ids as $id) {
                    if ($this->input->post('mass_delete')) {
                        if ($this->policies_model->delete_contact($id)) {
                            $total_deleted++;
                        }
                    } else {
                        if (!is_array($groups)) {
                        ...
                        $this->policies_model->handle_update_groups($id, $groups);
                    }
                }
            }
        }
            ...
            if (is_array($ids)) {
                foreach ($ids as $id) {
                    ...
                    if($fieldType=="standard"){
                        ...
                        $this->policies_model->update_contact($data,$id);
                    }
                    ...
                    if($fieldType=="related_to_company"){
                        ...
                        if($search_client){
                            ...
                            $contact_primary = $this->policies_model->get_contact_primary($search_client->userid);
                            $this->policies_model-> update_move_contact ($id,$search_client->userid,$contact_primary?0:1);
                        }else{
                            $data=array();
                            $data["company"] = $client_name;
                            $client_id = $this->policies_model->add($data);
                            $contact_primary = $this->policies_model->get_contact_primary($client_id);
                            $this->policies_model-> update_move_contact ($id,$client_id,$contact_primary?0:1);
                        }
                    }
                    $total_updated++;
                }
    ...
    public function vault_entry_create($customer_id)
    {
        ...
        $this->policies_model->vault_entry_create($data, $customer_id);
    ...
    {
        $entry = $this->policies_model->get_vault_entry($entry_id);
        ...
        if ($entry->creator == get_staff_user_id() || is_admin()) {
            ...
            $this->policies_model->vault_entry_update($entry_id, $data);
    ...
    {
        $entry = $this->policies_model->get_vault_entry($id);
        if ($entry->creator == get_staff_user_id() || is_admin()) {
            $this->policies_model->vault_entry_delete($id);
        }
        redirect($_SERVER['HTTP_REFERER']);
    }
    ...
    public function vault_encrypt_password()
    {
        ...
        $vault = $this->policies_model->get_vault_entry($id);
    ...
    public function statement_pdf()
    {
        ...
        $data['statement'] = $this->policies_model->get_statement($customer_id, to_sql_date($from), to_sql_date($to));
    ...
    public function send_statement()
    {
        ...
        $success = $this->policies_model->send_statement_to_email($customer_id, $send_to, $from, $to, $cc);
