...
class Misc extends Admin_controller
{
    ...
    {
        $result[] = $this->misc_model->_search_companies($this->input->post('q'),20);
        $data['result'] = $result;
        $data['tag'] = $this->input->post('tag');
        $this->load->view('admin/search_company', $data);
    }

    public function search_contact()
    {
        $result[] = $this->misc_model->_search_contact_byname($this->input->post('q'),20);
        $data['result'] = $result;
        $data['tag'] = $this->input->post('tag');
        //var_dump($data);die;
        $this->load->view('admin/search_contact', $data);
    }

    public function search_company_related_to()
    {
        $result[] = $this->misc_model->_search_companies($this->input->post('q'),20);
        $data['result'] = $result;
        $data['tag'] = $this->input->post('tag');
        $this->load->view('admin/search_company_related_to', $data);
    }

    public function search_contact_related_to()
    {
        $result[] = $this->misc_model->_search_contacts_related_to($this->input->post('q'),20);
        $data['result'] = $result;
        $data['tag'] = $this->input->post('tag');
        $this->load->view('admin/search_contact_related_to', $data);
    }
        ...
        if ($this->input->post()) {
            ...
            if ($success) {
                ...
                if($rel_type=='contacts'){
                    ...
                    $this->clients_model->log_contact_activity($rel_id, 'not_contact_activity_note_created', false, serialize(array(
    ...
    public function edit_note($id,$contact_id='')
    {

        if ($this->input->post()) {
            $success = $this->misc_model->edit_note($this->input->post(), $id);
            if($success){
                $note  = $this->misc_model->get_note($id);
                if($note->rel_type=='contacts'){
                    $this->load->model('clients_model');
                    if($contact_id){
                        $this->clients_model->log_contact_activity($contact_id, 'not_contact_activity_note_updated', false, serialize(array(
                            get_staff_full_name(get_staff_user_id()),nl2br($this->input->post()['description'])
                        )));
                    }
    ...
    {
        $success = $this->misc_model->delete_call_log($id);
