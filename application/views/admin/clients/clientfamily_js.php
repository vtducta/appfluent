...
$(function() {
    ...
    $('input[name="client[company]"]').on('keyup paste', function() {
        var q = $(this).val().trim();
...
function delete_contact_profile_image(contact_id) {
    requestGet('client_families/delete_contact_profile_image/'+contact_id).done(function(){
...
function update_all_proposal_emails_linked_to_contact(contact_id) {
    ...
    $.post(admin_url + 'client_families/update_all_proposal_emails_linked_to_customer/' + contact_id, data).done(function(response) {
        response = JSON.parse(response);
        if (response.success) {
            alert_float('success', response.message);
        }
