...
$(function() {
    ...
    $('input[name="policy[company]"]').on('keyup paste', function() {
        var q = $(this).val().trim();
        var tag_id= $(this).attr('id');

        var search_results = $('#company_search_results');
        ...
        delay(function() {
            ...
            $.post(admin_url + 'misc/search_company', { q: q,tag: tag_id }).done(function(results) {
                content_wrapper.unhighlight();
    ...
    $('input[name="policy[contact]"]').on('keyup paste', function() {
        var q = $(this).val().trim();
        var tag_id= $(this).attr('id');

        var search_results = $('#contact_search_results');
        ...
        delay(function() {
            if (q == original_top_search_val) { return; }
            $.post(admin_url + 'misc/search_contact', { q: q,tag: tag_id }).done(function(results) {
                content_wrapper.unhighlight();
                search_results.html(results);
                content_wrapper.highlight(q);
                original_top_search_val = q;
    ...
    var contactid = get_url_param('contactid');
    if (contactid) {
   ...
   initDataTable('.table-credit-notes', admin_url+'credit_notes/table/'+customer_id, ['undefined'], ['undefined'], undefined, [0, 'DESC']);
            ...
            'contact[email]': {
                ...
                remote: {
                    ...
                    data: {
                        ...
                        userid: function() {
                            return $('body').find('input[name="contact[contactid]"]').val();
                        }
                    }
                }
                <?php } ?>
    ...
    $('.billing-same-as-customer').on('click', function(e) {
        e.preventDefault();
        $('textarea[name="client[billing_street]"]').val($('textarea[name="client[address]"]').val());
        $('input[name="client[billing_city]"]').val($('input[name="client[city]"]').val());
        $('input[name="client[billing_state]"]').val($('input[name="client[state]"]').val());
        $('input[name="client[billing_zip]"]').val($('input[name="client[zip]"]').val());
        $('select[name="client[billing_country]"]').selectpicker('val', $('select[name="client[country]"]').selectpicker('val'));
    });

    $('.customer-copy-billing-address').on('click', function(e) {
        e.preventDefault();
        $('textarea[name="client[shipping_street]"]').val($('textarea[name="client[billing_street]"]').val());
        $('input[name="client[shipping_city]"]').val($('input[name="client[billing_city]"]').val());
        $('input[name="client[shipping_state]"]').val($('input[name="client[billing_state]"]').val());
        $('input[name="client[shipping_zip]"]').val($('input[name="client[billing_zip]"]').val());
        $('select[name="client[shipping_country]"]').selectpicker('val', $('select[name="client[billing_country]"]').selectpicker('val'));
    });

    $('body').on('hidden.bs.modal', '#policy', function() {
        $('#policy_data').empty();
    });

    $('.client-form').on('submit', function() {
        $('select[name="client[default_currency]"]').prop('disabled', false);
    });

});

function set_company_tag(tag_id,id,value) {
    var hide_tag_id = tag_id+"_id";
    $('input[id="'+tag_id+'"]').val(value);
    //$('input[id="'+hide_tag_id+'"]').val(id);
}

 function set_contact_tag(tag_id,id,value) {
     $('input[id="'+tag_id+'"]').val(value);
     $('input[name="policy[contact_id]"]').val(id);
 }

function delete_policy_profile_image(policy_id) {
    requestGet('policies/delete_policy_profile_image/'+policy_id).done(function(){
        $('body').find('#policy-profile-image').removeClass('hide');
        $('body').find('#policy-remove-img').addClass('hide');
        $('body').find('#policy-img').attr('src', '<?php echo base_url('assets/images/user-placeholder.jpg'); ?>');
    });
}
...
function update_all_proposal_emails_linked_to_policy(policy_id) {
    var data = {};
    data.update = true;
    data.original_email = $('body').find('#policy_update_proposals_emails').data('original-email');
    $.post(admin_url + 'policies/update_all_proposal_emails_linked_to_customer/' + policy_id, data).done(function(response) {
        response = JSON.parse(response);
        if (response.success) {
            alert_float('success', response.message);
        }
...
function fetch_lat_long_from_google_cprofile() {
    var data = {};
    data.address = $('textarea[name="client[address]"]').val();
    data.city = $('input[name="client[city]"]').val();
    data.country = $('select[name="client[country]"] option:selected').text();
    ...
    $.post(admin_url + 'misc/fetch_address_info_gmaps', data).done(function(data) {
        ...
        if (data.response.status == 'OK') {
            $('input[name="client[latitude]"]').val(data.lat);
            $('input[name="client[longitude]"]').val(data.lng);
        } else {
