			...
			<div class="col-md-12">
				<div class="panel_s">
					<div class="panel-body">
						<div class="row">
							<div class="col-md-6">
								<?php if(!isset($project_id) && !isset($policy)){ ?>
									<a href="#" id="ticket_no_policy"><span class="label label-default">
										<i class="fa fa-envelope"></i> <?php echo _l('ticket_create_no_policy'); ?>
									</span>
								</a>
								<a href="#" class="hide" id="ticket_to_policy"><span class="label label-default">
									<i class="fa fa-user-o"></i> <?php echo _l('ticket_create_to_policy'); ?>
								</span>
							</a>
							<div class="mbot15"></div>
						<?php } ?>
						<?php echo render_input('subject','ticket_settings_subject','','text',array('required'=>'true')); ?>
						<div class="form-group select-placeholder" id="ticket_policy_w">
							<label for="policyid"><?php echo _l('policy'); ?></label>
							<select name="policyid" required="true" id="policyid" class="ajax-search" data-width="100%" data-live-search="true" data-none-selected-text="<?php echo _l('dropdown_non_selected_tex'); ?>">
								<?php if(isset($policy)) { ?>
									<option value="<?php echo $policy['id']; ?>" selected><?php echo $policy['firstname'] . ' ' .$policy['lastname']; ?></option>
								<?php } ?>
								<option value=""></option>
							</select>
...
<script>
	$(function(){
		...
		init_ajax_search('policy','#policyid.ajax-search',{
			tickets_policies:true,
			policy_userid:function(){
					// when ticket is directly linked to project only search project client id policies
		...
		<?php if(isset($project_id) || isset($policy)){ ?>
			$('body.ticket select[name="policyid"]').change();
		<?php } ?>
		<?php if(isset($project_id)){ ?>
